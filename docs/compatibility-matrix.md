...
Orbitra 的 mGBA 线程核心要求页面启用跨源隔离，并依赖 SharedArrayBuffer、WebAssembly、Canvas 2D 和 IndexedDB。应用优先使用 WebGL 2；不可用时自动使用 Canvas 2D 软件渲染。发布前请在目标浏览器打开应用，点击顶部「环境检查」，记录实际图形后端和完整操作结果；面板中的存储空间只表示浏览器当前估算值。
...
| 2026-09-16 | Chrome 151 / Ubuntu 22.04.5                    | Vite localhost，COOP / COEP；Playwright 强制 WebGL 1 / 2 均不可用，Canvas 2D 软件渲染              | 原创 GBA / GB / GBC 均产生非空画面，实测约 58 / 58 / 59 FPS；即时存档、截图、跨平台切换、worker 释放和同画布重建通过；完全禁用 Canvas 2D 时仍明确阻断并保留已有存档                                                   | 软件禁用为注入场景；未代表移动设备或所有低性能 CPU；音频仅验证程序状态，未人工试听                         |
...
| 2026-09-16 | Chrome 151.0.7922.137 / Linux                  | Vite localhost，COOP / COEP；Playwright headless + SwiftShader；运行时生成的原创 GB / GBC MBC1 ROM | GBA、GB、GBC 真实 mGBA 回归通过；GB / GBC 均为 160×144、约 58 FPS，方向输入、8 KiB 电池存档、即时存档、10:9 UI、平台输入、备份匹配和 worker 释放通过；121 项单元测试及 PWA 离线回归通过                               | 只覆盖原创 MBC1 + 8 KiB RAM 样本；其他 mapper、RTC、商业 ROM、实体设备、真实 GPU、音频试听和移动安装未验证 |
...
备份回归可使用 `pnpm test:backup`；支持上述 `UI_TEST_URL` 与 `BROWSER_EXECUTABLE_PATH`。GBA 流程使用原创 Star Orbit 及其头部变体，GB / GBC 流程使用运行时生成的原创最小 ROM，不引入商业内容。损坏 ZIP、哈希、引用关系、平台身份、大小限制与事务竞态还由 121 项单元测试中的相应格式 / 存储测试验证。格式限制为软件边界，不作为设备内存能力证明。
