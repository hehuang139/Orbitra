...
[效果预览](#效果预览) · [功能特性](#功能特性) · [快速开始](#快速开始) · [部署](#构建与部署) · [品牌素材](docs/brand.md) · [Roadmap](ROADMAP.md) · [参与贡献](CONTRIBUTING.md) · [English](README.md)
...
游戏内的即时存档面板提供 **1 个自动槽与 5 个手动槽**。自动槽默认每 10 分钟更新，也可改为每 1 或 5 分钟，并始终原位覆盖，不会不断增加副本；也可保存、读取、导入和导出即时存档，备份或导入游戏内的 `.sav` 存档。
...
移动端布局按平台提供方向键、A / B、Start / Select、L / R 与 SFC 的 X / Y 触屏控件；播放工具栏也可直接操作。可选择标准或紧凑布局、独立面板或画面遮罩，调整按键大小与不透明度；全屏会自动使用遮罩布局，支持多指同时操作，横竖屏切换会释放当前按键。
...
| **进度管理**   | 自动存档可选每 1 / 5 / 10 分钟，并在返回游戏库、切入后台时保存；固定覆盖 1 个自动槽；支持 `.sav` 导入 / 导出 |
| **批量备份**   | 选择游戏导出带校验和的 ZIP，默认不含 ROM；恢复前预览、匹配缺失 ROM、逐项选择冲突，失败整体回滚               |
| **账号同步**   | 可选登录；同步单个用户自己的游戏清单、ROM、游戏内存档与即时存档，新浏览器登录后恢复                          |
| **在线游戏库** | 配置独立分发地址；按平台浏览和导入公开目录，管理员可从个人游戏库或文件发布 ROM，也可下架 ROM，不接收个人存档 |
| **Minecraft**  | 浏览 Mojang 官方 Java 版目录并校验管理；1.2.5 可在浏览器试玩，本机伴侣可尝试启动现代版本                     |
| **播放控制**   | 暂停、继续、重置、全屏；1× / 2× / 4× 速度；按住快进与倒带；音量、静音                                        |
| **输入方式**   | 可重映射键盘；按设备保存手柄按钮 / 轴方向与死区；标准 / 紧凑触屏布局、独立面板 / 画面遮罩、大小与不透明度    |
| **金手指**     | 按游戏保存命名代码与启用状态；由 mGBA、FCEUmm、Snes9x 原生核心执行，并随备份与账号同步                       |
...
触屏配置提供标准 / 紧凑布局、独立面板 / 画面遮罩、80%–130% 按键大小和 40%–100% 不透明度，刷新后保留；旧配置自动补齐默认值。窄屏会限制实际按键尺寸，样式预留安全区。手柄断连、窗口失焦、弹窗、触控取消和暂停会释放相应输入；多个输入来源共同按住同一按键时，释放其中一个不会中断其余来源。实体手机、手柄和辅助技术的验证范围见 [兼容性记录](docs/compatibility-matrix.md)。
...
在线库默认监听 [http://localhost:4174](http://localhost:4174)，并从 `public/demo` 生成公开分发目录。侧栏「在线游戏库」包含“浏览与导入”和“管理分发”两个视图；前者可按游戏平台筛选目录，后者使用 `ONLINE_LIBRARY_ADMIN_TOKEN` 从个人游戏库直接发布 ROM、选择本地 ROM 文件发布或下架 ROM。从个人游戏库发布只读取所选 ROM，不上传存档。通过 `ONLINE_LIBRARY_HOST`、`ONLINE_LIBRARY_PORT`、`ONLINE_LIBRARY_ROOT`、`ONLINE_LIBRARY_NAME` 和 `ONLINE_LIBRARY_ALLOWED_ORIGINS` 配置服务；HTTPS 使用 `ONLINE_LIBRARY_TLS_CERT_PATH` 与 `ONLINE_LIBRARY_TLS_KEY_PATH`。请定期备份分发目录。
...
- [x] 增加 Minecraft: Java Edition 官方版本库，支持发现、下载并校验官方清单仍公开的各版本；1.2.5 可通过 CheerpJ 在浏览器内受限试玩。现代版本可通过一次安装和配对的 Orbitra Companion 尝试本机启动；发布仍需自有 Microsoft OAuth 客户端、签名与真实设备验证。详细范围见 [Minecraft roadmap](docs/roadmaps/minecraft.md) 和 [实现边界](docs/minecraft-support.md)。
