...
import type { LibraryChange, RestoreChoices, RestorePreview, RomData } from '../lib/storage.ts'
      ...
      try {
        ...
        if (pullRemote) {
          ...
          if (index) {
            ...
            const localGameCube = games.filter((game) => game.platform === 'gamecube').length
            setPhase('idle')
            setMessage(
              `已同步 ${games.length - localGameCube} 个游戏清单及存档${localGameCube ? `；${localGameCube} 个 GameCube 游戏仅保存在本机` : ''}`,
            )
            return
          }
        }
        ...
        const localOnly = localOnlyIds.size
        setMessage(
          waiting
            ? `已同步 ${games.length - localOnly} 个游戏；${waiting} 个旧游戏等待源浏览器更新清单`
            : `已同步 ${games.length - localOnly} 个游戏清单及存档${localOnly ? `；${localOnly} 个 GameCube 游戏仅保存在本机` : ''}`,
        )
      } catch (cause) {
        setPhase('error')
