...
It listens at [http://localhost:4174](http://localhost:4174) and distributes supported ROMs from `public/demo` by default. Open “Online game library” as a workspace page to search filenames, filter by platform, and browse/import the catalog. “Manage distribution” uses `ONLINE_LIBRARY_ADMIN_TOKEN` to publish a ROM directly from the personal game library, publish a selected local file, or remove a file. Publishing from the personal library reads only the selected ROM and never uploads its saves. Configure `ONLINE_LIBRARY_HOST`, `ONLINE_LIBRARY_PORT`, `ONLINE_LIBRARY_ROOT`, `ONLINE_LIBRARY_NAME` and `ONLINE_LIBRARY_ALLOWED_ORIGINS`; set both `ONLINE_LIBRARY_TLS_CERT_PATH` and `ONLINE_LIBRARY_TLS_KEY_PATH` for HTTPS. Back up the configured distribution directory.
...
Gamepad and touch customization and keyboard focus improvements are implemented. Remaining work includes physical-device and screen-reader verification, broader browser / low-end-device evidence, UI localization and more redistributable homebrew tests. Backup and restore software is implemented; its remaining device and memory validation is tracked in the [v1.2 roadmap](docs/roadmaps/v1.2.md). Minecraft: Java Edition support discovers, downloads, deduplicates, verifies, repairs, and removes versions still published in Mojang's official catalog. A constrained CheerpJ browser demonstration is available for 1.2.5. Modern versions can be attempted through the separately installed Orbitra Companion; public releases still require an Orbitra-owned Microsoft OAuth client, signed installers, and real-device validation. See the [Minecraft roadmap](docs/roadmaps/minecraft.md), [implementation boundaries](docs/minecraft-support.md), [full roadmap](ROADMAP.md), and [changelog](CHANGELOG.md). The v1.1 validation work remains open where devices or manual evidence are missing; unchecked items are not release-date commitments.
