...
一个现代、美观的 GBA、GB 与 GBC 浏览器模拟器。真实 mGBA 内核，本地游戏库，随时保存，再次出发。
...
Advance 把熟悉的掌机体验带到浏览器：整理 GBA、GB 与 GBC 游戏，连接手柄、回到刚才的存档，或在手机上继续探索。界面采用中文设计，适配桌面、平板与手机；游戏由 **mGBA WebAssembly** 核心实际执行，包含图像、声音与游戏内存档。
...
内置使用指南涵盖 GBA / GB / GBC / ZIP 导入、键盘操作与存档方式，并集中列出暂停、快速存取档、快进、倒带和全屏快捷键。
...
移动端布局提供方向键、A / B、Start / Select 等触屏控件，GBA 游戏还会显示 L / R；播放工具栏也可直接操作。可选择标准或紧凑布局，调整按键大小与不透明度；支持多指同时操作，横竖屏切换会释放当前按键。
...
| 模块         | 已实现功能                                                                                                              |
| ------------ | ----------------------------------------------------------------------------------------------------------------------- |
| **真实模拟** | mGBA WASM 核心；GBA、GB、GBC 画面、音频与游戏内存档                                                                     |
| **游戏库**   | 多文件与拖放导入；平台标签与筛选；搜索、排序、网格 / 列表、收藏、最近游玩、累计时长、删除                               |
| **ZIP 导入** | 自动读取子目录中的 `.gba`、`.gb`、`.gbc`；一次导入多款游戏；平台域 SHA-256 内容去重，保留已有收藏与进度并隔离跨平台存档 |
...
| **画面风格** | GBA 3:2 与 GB / GBC 10:9 原生比例；像素、平滑、CRT 扫描线；真实核心帧缓冲截图                                           |
...
打开 [http://localhost:5173](http://localhost:5173)，点击「开始试玩」，或导入自己的 `.gba`、`.gb`、`.gbc` / `.zip` 文件。
...
标准手柄默认支持 A / B、L / R、Start / Select、十字键和左摇杆；GB / GBC 游戏不使用 L / R。浏览器通常需要先按一次手柄按钮才能识别。在「控制器设置」选择设备后，点击某个模拟按键，再按手柄按钮或推动摇杆即可录入；可取消、清除单项、恢复默认，并在 10%–90% 范围调整摇杆死区。非标准手柄初始为空映射，需要自行设置。配置按浏览器提供的设备标识和布局保存，重新连接后的插槽变化不影响配置；相同标识和布局的设备共用配置。
...
| 子目录与重复文件    | 递归识别三种 ROM，忽略说明文档与 macOS 元数据；同内容游戏自动合并 |
...
浏览器需支持 SharedArrayBuffer、WebAssembly 线程、WebGL 和 IndexedDB。首次访问需要联网缓存应用壳；之后可在已缓存资源范围内离线打开，并通过 Service Worker 的用户确认流程更新资源。
...
flowchart LR
    ...
    B --> D
    E[GBA / GB / GBC / ZIP 文件] --> F[平台识别 · 校验 · 解压 · 内容去重]
    F --> D
```
...
| 多平台流程   | GBA / GB / GBC 导入、筛选、标签、原生比例、平台输入与刷新恢复                                     |
...
浏览器测试默认连接 `http://127.0.0.1:5173`。内核、手柄与启动套件使用 Vite 开发测试页或模块插桩，需要 `pnpm dev`。环境变量、测试约定与贡献流程见 [CONTRIBUTING.md](CONTRIBUTING.md)。模拟输入和手机视口只提供自动化证据，不能替代实体设备、音频试听或屏幕阅读器检查。测试使用原创试玩 ROM 与运行时生成的原创最小 GB / GBC ROM；GB / GBC 自动化样本覆盖 MBC1 与 8 KiB 电池 RAM，不代表其他 mapper、RTC 或完整商业 ROM 兼容性。

## Roadmap

当前版本已提供 GBA / GB / GBC 导入、实际模拟、存取档、备份恢复与多种输入方式。下面同时列出已交付基础和仍待完成的验证：
...
优先级、验收方向与待评估功能见 [完整 Roadmap](ROADMAP.md)，已发布版本的变化见 [更新日志](CHANGELOG.md)。目前不支持 FC / SFC、联机、作弊码或云同步；路线图不代表这些功能已可用，也不承诺发布日期。
